...
(defun validate (data &key (schema-version *schema-version*) (pretty-errors-p t) schema)
  "The primary validation function for json-schema.  Takes a schema of type hash-table, data: which can be a simple value or an object as a hash table, and then optionally accepts a schema version and pretty-errors-p deterimines whether the second return value is exception objects or strings of the rendered errors (strings by default)."
