(defpackage :rove/reporter/junit
  (:local-nicknames (:self :rove/reporter/junit))
  (:use #:cl
        #:rove/reporter
        #:rove/core/stats
        #:rove/core/result
...
(defmethod record ((reporter junit-reporter) (object failed-assertion))
  ...
  (let ((stream (junit-stream-of reporter)))
    ...
    (with-indent (stream +2)
      (fresh-line stream)
      (format stream
              "<testcase name=\"~/self::xml-escape-printer/\" ~
                         classname=\"~/self::xml-escape-printer/\" ~
                         time=\"~f\">"
      ...
      (with-indent (stream +2)
        ...
        (if (null (assertion-stacks object))
            ...
            (progn
              (format stream "<failure type=\"~/self::xml-escape-printer/\">~/self::xml-escape-printer/</failure>"
                      (string (type-of (assertion-reason object)))
                      (string (assertion-reason object))))
            ;; an error with a backtrace
            (progn
              (format stream "<error type=\"~/self::xml-escape-printer/\">~{~/self::xml-escape-printer/~^~%~}</error>"
                      (string (type-of (assertion-reason object)))
  ...
  (when (<= *test-depth* 2)
    (let ((stream (junit-stream-of reporter)))
      ...
      (if (toplevel-stats-p reporter)
          ...
          (format stream
                  "<testsuite name=\"~/self::xml-escape-printer/\" ~
                            timestamp=\"~a\" ~
                            time=\"~d\" ~
                            hostname=\"~a\" ~
                            failures=\"~d\" ~
...
(defun run (component &key (style rove/main:*default-reporter*))
  (if (uiop:getenvp "JUNIT_ENABLED")
      (with-open-file (stream (if (uiop:getenvp "JUNIT_REPORT_FILENAME")
        ...
        (let ((*junit-output* stream))
          (rove:run component :style :junit)))
      (rove:run component :style style)))
